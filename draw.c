...
struct matrix * generate_sphere(double cx, double cy, double cz,
                                double r, int step ) {
  struct matrix * sphere = new_matrix(4, 255);
  double theta;
  double phi;
  int i;
  int j;
  ...
  for(i = 0; i < (step + 1); i++){
    theta = 2 * M_PI * ((double) i) / step;
    for(j = -1 * step / 2; j < step / 2; j++){
      phi = 2 * M_PI * ((double) j) / step;
      add_point(sphere, r * cos(phi) * cos(theta) + cx, r * cos(phi) * sin(theta) + cy, r * sin(phi) + cz);
    }
  }
...
struct matrix * generate_torus( double cx, double cy, double cz,
                                double r1, double r2, int step ) {
  struct matrix * torus = new_matrix(4, 255);
  double theta;
  double phi;
  int i;
  int j;
  for(i = 0; i < (step + 1); i++){
    ...
    for(j = 0; j < (step + 1); j++){
      phi = 2 * M_PI * ((double) j) / step;
      add_point(torus, r1 * cos(theta) * cos(phi) + r2 * cos(phi) + cx, r1 * cos(theta) * sin(phi) + r2 * sin(phi) + cy, r1 * sin(theta) + cz);
    }
  }
  return torus;
}
